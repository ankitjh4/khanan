#!/usr/bin/env python3
"""Build KHANAN Alpha 3.0 release contracts and consolidated gap disclosure.

The release-artifact manifest freezes the schema of every published CSV plus
the public auction-footprint and ranked-candidate GeoJSON files. The coverage-gap register preserves one
...
SCHEMA_FREEZE_STATUS = "frozen_for_v1.0-alpha.30"
...
EXPECTED_SCHEMA_SHA256 = {
    ...
    "india_mining_candidate_areas_validation_gated.geojson": "3a1258b86de5a73a4ad1c8d5a0bb26ea2a07b40c8b746a8049f7eeccaf5d3fea",
...
CONTRACTS = {
    ...
    "india_mining_candidate_areas_validation_gated.geojson": contract(["record_id"], "model_hypothesis", "production_model_output_v0.6_geospatial_view", "One validation-gated H3 candidate polygon with typed CSV properties and a source-row hash.", "National H3 grid; promoted subset", "Feature reference periods vary; model v0.6; Alpha 3.0 serialization", "scripts/build_candidate_geojson.py", "outputs/alpha3_completion_audit.json", "A geospatial serialization of unchanged model hypotheses; not discoveries, reserves, resources, grades, legal rights or drill targets."),
...
STATIC_SOURCE_IDS = {
    ...
    "india_mining_candidate_areas_validation_gated.geojson": BASE_SOURCES,
...
CROSS_CUTTING_GAPS = [
    cross_gap("GAP-PROJECT-001", ["material_coverage", "model_support"], "material ontology and prediction eligibility", ["india_material_ontology_v1.csv", "india_strategic_materials_top50.csv", "material_model_support.csv"], "India", "Alpha 3.0 snapshot", "Ontology coverage is much broader than prediction eligibility and validated scoring support.", "254 ontology entities; 50 model targets; 34 with MRDS evidence; 20 with percentiles; 15 spatially validated; 14 passing minimum holdout AUC.", "Most catalogued materials cannot receive defensible rankings.", "Unsupported materials remain explicitly unscored.", "Independent coordinate-bearing positives and deposit-type labels sufficient for spatial holdout.", "No automatic score is created from ontology membership.", "outputs/validation_report.json and outputs/material_ontology_validation_v1.json"),
...
def build_manifest(source_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for name in sorted(CONTRACTS):
        ...
        artifact_id = f"{Path(name).stem}_geojson" if name.endswith(".geojson") else Path(name).stem
        rows.append({
            "artifact_id": artifact_id,
            "relative_path": f"outputs/{name}",
            "artifact_type": "vector_geojson" if name.endswith(".geojson") else "tabular_csv",
            "release_version": RELEASE_VERSION,
            "schema_version": f"alpha30-schema-{actual_schema[:12]}",
...
def update_validation_report(validation: dict[str, Any]) -> None:
    ...
    report.pop("release_governance_alpha24", None)
    report.pop("release_governance_alpha25", None)
    report["release_governance_alpha30"] = {
...
def main() -> None:
    ...
    validation["checks_pass"] = all([
        len(manifest_rows) == len(CONTRACTS) == 45,
        len(csv_names) == 43,
        all_keys_unique,
        blank_key_rows == 0,
