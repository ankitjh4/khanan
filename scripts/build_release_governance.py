#!/usr/bin/env python3
"""Build KHANAN Alpha.25 release contracts and consolidated gap disclosure.
...
SCHEMA_FREEZE_STATUS = "frozen_for_v1.0-alpha.25"
...
CROSS_CUTTING_GAPS = [
    cross_gap("GAP-PROJECT-001", ["material_coverage", "model_support"], "material ontology and prediction eligibility", ["india_material_ontology_v1.csv", "india_strategic_materials_top50.csv", "material_model_support.csv"], "India", "Alpha.25 snapshot", "Ontology coverage is much broader than prediction eligibility and validated scoring support.", "254 ontology entities; 50 model targets; 34 with MRDS evidence; 20 with percentiles; 15 spatially validated; 14 passing minimum holdout AUC.", "Most catalogued materials cannot receive defensible rankings.", "Unsupported materials remain explicitly unscored.", "Independent coordinate-bearing positives and deposit-type labels sufficient for spatial holdout.", "No automatic score is created from ontology membership.", "outputs/validation_report.json and outputs/material_ontology_validation_v1.json"),
...
def build_manifest(source_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for name in sorted(CONTRACTS):
        ...
        rows.append({
            ...
            "schema_version": f"alpha25-schema-{actual_schema[:12]}",
...
def update_validation_report(validation: dict[str, Any]) -> None:
    ...
    report["release_governance_alpha25"] = {
