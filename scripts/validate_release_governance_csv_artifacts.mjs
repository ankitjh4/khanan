...
const outputDir = path.join(root, "outputs", "diagnostics", "release_governance_csv_artifact_qa");
...
const targets = [
  ...
  },
  {
    ...
    expectedRows: 58,
    expectedColumns: 17,
    ranges: ["A1:I16", "J1:Q16", "A44:I59", "J44:Q59"],
  },
  {
    file: "data_dictionary.csv",
    expectedRows: 1928,
    expectedColumns: 6,
    ranges: ["A1:F16", "A1860:F1929"],
  },
];
...
for (const target of targets) {
  ...
  for (const [rangeIndex, range] of target.ranges.entries()) {
    ...
    const preview = await workbook.render({ sheetName: "Data", range, scale: 1.15, format: "png" });
  ...
  summary.push({ file: target.file, rows: shape.rows, columns: shape.columns, inspections, formulaErrorScan: errors.ndjson, previews });
  if (global.gc) global.gc();
}

await fs.writeFile(path.join(outputDir, "summary.json"), `${JSON.stringify(summary, null, 2)}\n`);
console.log(JSON.stringify(summary.map(({ file, rows, columns, previews }) => ({ file, rows, columns, previews })), null, 2));
