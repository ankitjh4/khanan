...
- 44 release-artifact contracts covering 43 CSVs and one GeoJSON; frozen schema hashes; unique, nonblank primary keys; five explicit evidence roles; 1,928 dictionary definitions covering every CSV column; 38 source-specific and 20 project-wide coverage-gap rows; and unchanged national-grid and candidate hashes.

Build-time results are written to `outputs/validation_report.json`. Release-wide schema and coverage results are also written to `outputs/release_governance_validation.json`; the contract is documented in [`release_governance_alpha24.md`](release_governance_alpha24.md). A passing report means the pipeline's structural checks passed; it does not validate mineral occurrence in the field.
