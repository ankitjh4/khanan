...
- 45 release-artifact contracts covering 43 CSVs and two GeoJSON files; frozen schema hashes; unique, nonblank primary keys; five explicit evidence roles; 1,928 dictionary definitions covering every CSV column; 38 source-specific and 20 project-wide coverage-gap rows; and unchanged national-grid and candidate hashes.

Build-time results are written to `outputs/validation_report.json`. Release-wide schema and coverage results are also written to `outputs/release_governance_validation.json`; the contract is documented in [`release_governance_alpha24.md`](release_governance_alpha24.md). Alpha 3.0 additionally publishes `outputs/alpha3_completion_audit.json` and the evidence matrix in [`alpha3_final_release_audit.md`](alpha3_final_release_audit.md). The 2,784-row candidate CSV has an exact typed Polygon GeoJSON counterpart whose features carry hashes of their complete source rows. A passing report means the pipeline's structural checks passed; it does not validate mineral occurrence in the field.
