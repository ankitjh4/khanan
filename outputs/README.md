...
- The root contains the v0.8 geospatial feature baseline and unchanged v0.6 prospectivity model in the current v1.0-alpha.7 development release: the v1 ontology, IBM abandoned-mine context, metadata-only NGDR/GSI service audit, EMAG2v3 and SoilGrids context plus paired spatial ablations, crosswalk, validation, checksums and compressed bundle.
...
The full `india_mining_prospectivity_grid_h3_r6.csv` is intentionally excluded from the Git checkout because it exceeds GitHub's normal single-file limit. It remains available locally and inside `india_mining_dataset_csv_bundle_v1.0-alpha.7.zip`.
