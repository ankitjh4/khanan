...
The compressed `../outputs/india_mining_dataset_csv_bundle_v1.0-alpha.6.zip` is the current portable development release. It packages the v0.8 geospatial feature baseline and unchanged v0.6 production model together with the v1 ontology, source-term crosswalk, IBM abandoned-mine context layer, metadata-only NGDR/GSI service audit, uncertainty-aware NOAA/NCEI EMAG2v3 magnetic context, paired spatial-ablation evidence, and uncertainty-aware ISRIC SoilGrids 2.0 soil context. The uncompressed nationwide grid is regenerated locally and excluded from the Git checkout because it exceeds GitHub's ordinary file-size limit.
