...
**Development release: v1.0-alpha.10 · Geospatial feature baseline: v0.10 · Prospectivity model baseline: v0.6 · Active work in progress**
...
![KHANAN reviewed IBM auction Mine Block Summary geometries](assets/maps/khanan-ibm-auction-mbs-geometries-alpha10.png)

The fourth figure locates the ten IBM Table 5 blocks whose matched State Mine Block Summaries passed the alpha.10 geometry gates. It also makes the incomplete review scope explicit: seven reviewed records are withheld and 80 IBM rows remain unreviewed. The map is generated by `scripts/plot_ibm_auction_mbs_geometries.py`.
...
| `outputs/india_ibm_auctioned_concession_mbs_match_audit_2023_24.csv` | 97 | Complete IBM-row audit of the alpha.10 State MBS review scope, source profiles, coordinate-evidence class, selected documents, hashes, conflicts and geometry-admission outcomes. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.csv` | 10 | Validated source footprints with normalized materials, drilling, resources, grade, climate, terrain, hydrology, access, geometry checks and document provenance. Context only. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.geojson` | 10 | GeoJSON representation of the same ten validated source footprints. |
...
- **Extended in alpha.10:** link seventeen IBM 2023-24 auction rows to exact State Mine Block Summaries, admit ten validated source footprints, retain technical profiles for every reviewed document, and quarantine seven records with explicit failure reasons.
