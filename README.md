...
**Development release: v1.0-alpha.14 · Geospatial feature baseline: v0.11 · Prospectivity model baseline: v0.6 · Active work in progress**

The current release covers India with 88,857 H3 resolution-6 cells, approximately 36 km² each. It includes 781 public-source site records, 1,504 IBM MCDR inspection events, 82 IBM abandoned-mine inventory records, 722 IBM NMI resource-inventory rows, 123 IBM 2024 lease-distribution rows, 97 IBM auction-granted concession records, five dated Andhra auction-status evidence rows, 199 central critical-mineral auction events, 50 strategic modeling targets, and 2,784 validation-gated candidate cells.
...
![KHANAN reviewed IBM auction Mine Block Summary geometries](assets/maps/khanan-ibm-auction-mbs-geometries-alpha14.png)

The fourth figure locates the 24 IBM Table 5 blocks whose matched State Mine Block Summaries passed the geometry gates. Maharashtra adds seven admitted source footprints—five Surjagad iron-ore blocks, Minzhari copper and Savali manganese—while the Andhra Pradesh panel remains status evidence only. Fifty-three IBM rows remain unreviewed. The map is generated by `scripts/plot_ibm_auction_mbs_geometries.py`.
...
| IBM 2024 mining-lease and auction-granted concession context | Implemented as context; spatial and status reconciliation started | National/state/mineral/sector/area-band distributions and 97 named blocks. Alpha.14 reviews 44 rows: 39 exact State MBS selections plus five Andhra exact-name searches without a current public MBS match. Twenty-four validated source footprints are admitted, fifteen reviewed records are withheld, and 53 records remain unreviewed. Maharashtra adds seven admitted footprints, two extent-only withholdings and one published-area validation failure; all five Andhra rows retain dated official-secondary status evidence without inferred geometry or operating-status claims. |
...
| `outputs/india_ibm_auctioned_concession_mbs_match_audit_2023_24.csv` | 97 | Complete IBM-row audit of the alpha.14 State MBS review scope, source profiles, coordinate-evidence class, selected documents, missing-current-index matches, conflicts and geometry-admission outcomes. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.csv` | 24 | Validated source footprints with normalized materials, drilling, resources, grade, climate, terrain, hydrology, access, geometry checks and document provenance. Context only. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.geojson` | 24 | GeoJSON representation of the same 24 validated source footprints. |
...
| `outputs/source_registry.csv` | 34 | Provenance, access/licensing notes, uses and limitations. |
| `outputs/data_dictionary.csv` | 1,162 | Field definitions, units and missing-value policies. |
...
| `outputs/india_mining_dataset_csv_bundle_v1.0-alpha.14.zip` | — | Portable 81-member development bundle, including the full grid, reviewed State MBS geometry and Andhra status-evidence layers, magnetic and soil features, paired ablation evidence, ontology, contextual mine and concession layers, service audit, validations, maps and reproducibility scripts. |
...
- **Extended through alpha.14:** review 44 IBM 2023-24 auction rows, link 39 to exact State Mine Block Summaries, admit 24 validated source footprints, quarantine fifteen selected-MBS records with explicit failures, and publish dated status evidence for all five Andhra rows without inferring missing polygons.
