...
**Development release: v1.0-alpha.11 · Geospatial feature baseline: v0.11 · Prospectivity model baseline: v0.6 · Active work in progress**
...
![KHANAN reviewed IBM auction Mine Block Summary geometries](assets/maps/khanan-ibm-auction-mbs-geometries-alpha11.png)

The fourth figure locates the fourteen IBM Table 5 blocks whose matched State Mine Block Summaries passed the alpha.11 geometry gates. It also makes the incomplete review scope explicit: nine reviewed records are withheld and 74 IBM rows remain unreviewed. The map is generated by `scripts/plot_ibm_auction_mbs_geometries.py`.
...
| `outputs/india_ibm_auctioned_concession_mbs_match_audit_2023_24.csv` | 97 | Complete IBM-row audit of the alpha.11 State MBS review scope, source profiles, coordinate-evidence class, selected documents, hashes, conflicts and geometry-admission outcomes. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.csv` | 14 | Validated source footprints with normalized materials, drilling, resources, grade, climate, terrain, hydrology, access, geometry checks and document provenance. Context only. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.geojson` | 14 | GeoJSON representation of the same fourteen validated source footprints. |
...
- **Extended through alpha.11:** link 23 IBM 2023-24 auction rows to exact State Mine Block Summaries, admit fourteen validated source footprints, retain technical profiles for every reviewed document, and quarantine nine records with explicit failure reasons.
