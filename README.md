...
**Development release: v1.0-alpha.15 · Geospatial feature baseline: v0.11 · Prospectivity model baseline: v0.6 · Active work in progress**
...
![KHANAN reviewed IBM auction Mine Block Summary geometries](assets/maps/khanan-ibm-auction-mbs-geometries-alpha15.png)

The fourth figure locates the 39 IBM Table 5 blocks whose matched State Mine Block Summaries passed the geometry gates. Madhya Pradesh adds fifteen admitted Phase-XI footprints spanning limestone, manganese, phosphorite, copper, graphite and vanadium context; seven additional Madhya Pradesh records remain visible in the audit as reviewed withholds. The Andhra Pradesh panel remains status evidence only, and 31 Rajasthan rows remain unreviewed. The map is generated by `scripts/plot_ibm_auction_mbs_geometries.py`.
...
| IBM 2024 mining-lease and auction-granted concession context | Implemented as context; spatial and status reconciliation started | National/state/mineral/sector/area-band distributions and 97 named blocks. Alpha.15 reviews 66 rows: 61 exact State MBS selections plus five Andhra exact-name searches without a current public MBS match. Thirty-nine validated source footprints are admitted, 22 selected-MBS records are withheld, five Andhra rows retain status evidence without inferred geometry, and 31 Rajasthan records remain unreviewed. |
...
| `outputs/india_ibm_auctioned_concession_mbs_match_audit_2023_24.csv` | 97 | Complete IBM-row audit of the alpha.15 State MBS review scope, source profiles, coordinate-evidence class, selected documents, missing-current-index matches, conflicts and geometry-admission outcomes. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.csv` | 39 | Validated source footprints with normalized materials, drilling, resources, grade, climate, terrain, hydrology, access, geometry checks and document provenance. Context only. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.geojson` | 39 | GeoJSON representation of the same 39 validated source footprints. |
...
- **Extended through alpha.15:** review 66 IBM 2023-24 auction rows, link 61 to exact State Mine Block Summaries, admit 39 validated source footprints, quarantine 22 selected-MBS records with explicit failures, and publish dated status evidence for all five Andhra rows without inferring missing polygons.
