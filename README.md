...
**Development release: v1.0-alpha.12 · Geospatial feature baseline: v0.11 · Prospectivity model baseline: v0.6 · Active work in progress**
...
![KHANAN reviewed IBM auction Mine Block Summary geometries](assets/maps/khanan-ibm-auction-mbs-geometries-alpha12.png)

The fourth figure locates the seventeen IBM Table 5 blocks whose matched State Mine Block Summaries passed the alpha.12 geometry gates. It also makes the incomplete review scope explicit: twelve reviewed records are withheld and 68 IBM rows remain unreviewed. The map is generated by `scripts/plot_ibm_auction_mbs_geometries.py`.
...
| IBM 2024 mining-lease and auction-granted concession context | Implemented as context; spatial reconciliation started | National/state/mineral/sector/area-band distributions and 97 named blocks. Alpha.12 reviews 29 exact State MBS matches, admits seventeen validated source footprints and withholds twelve records with explicit reasons. All remain excluded from model evidence and current-status inference. |
...
| `outputs/india_ibm_auctioned_concession_mbs_match_audit_2023_24.csv` | 97 | Complete IBM-row audit of the alpha.12 State MBS review scope, source profiles, coordinate-evidence class, selected documents, hashes, conflicts and geometry-admission outcomes. |
...
| `outputs/source_registry.csv` | 32 | Provenance, access/licensing notes, uses and limitations. |
| `outputs/data_dictionary.csv` | 1,127 | Field definitions, units and missing-value policies. |
...
| `outputs/india_mining_dataset_csv_bundle_v1.0-alpha.12.zip` | — | Portable 77-member development bundle, including the full grid, reviewed State MBS geometry layer, magnetic and soil features, paired ablation evidence, ontology, contextual mine and concession layers, service audit, validations, maps and reproducibility scripts. |
...
- **Extended through alpha.12:** link 29 IBM 2023-24 auction rows to exact State Mine Block Summaries, admit seventeen validated source footprints, retain technical profiles for every reviewed document, and quarantine twelve records with explicit failure reasons.
