...
**Development release: v1.0-alpha.13 · Geospatial feature baseline: v0.11 · Prospectivity model baseline: v0.6 · Active work in progress**
...
![KHANAN reviewed IBM auction Mine Block Summary geometries](assets/maps/khanan-ibm-auction-mbs-geometries-alpha13.png)

The fourth figure locates the seventeen IBM Table 5 blocks whose matched State Mine Block Summaries passed the geometry gates. It adds an explicit Andhra Pradesh status-only panel: five rows have dated official-secondary evidence, but no public boundary document was found and no polygon is inferred. Sixty-three IBM rows remain unreviewed. The map is generated by `scripts/plot_ibm_auction_mbs_geometries.py`.
...
| IBM 2024 mining-lease and auction-granted concession context | Implemented as context; spatial and status reconciliation started | National/state/mineral/sector/area-band distributions and 97 named blocks. Alpha.13 reviews 34 rows: 29 exact State MBS selections plus five Andhra exact-name searches without a current public MBS match. Seventeen validated source footprints are admitted, twelve selected-MBS records are withheld, and all five Andhra rows receive dated official-secondary status evidence without inferred geometry or operating-status claims. |
...
| `outputs/india_ibm_auctioned_concession_mbs_match_audit_2023_24.csv` | 97 | Complete IBM-row audit of the alpha.13 State MBS review scope, source profiles, coordinate-evidence class, selected documents, missing-current-index matches, conflicts and geometry-admission outcomes. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.csv` | 17 | Validated source footprints with normalized materials, drilling, resources, grade, climate, terrain, hydrology, access, geometry checks and document provenance. Context only. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.geojson` | 17 | GeoJSON representation of the same seventeen validated source footprints. |
...
- **Extended through alpha.13:** review 34 IBM 2023-24 auction rows, link 29 to exact State Mine Block Summaries, admit seventeen validated source footprints, quarantine twelve selected-MBS records with explicit failures, and publish dated status evidence for all five Andhra rows without inferring missing polygons.
