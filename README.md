...
**Development release: v1.0-alpha.9 · Geospatial feature baseline: v0.9 · Prospectivity model baseline: v0.6 · Active work in progress**
...
![KHANAN reviewed IBM auction Mine Block Summary geometries](assets/maps/khanan-ibm-auction-mbs-geometries-alpha9.png)

The fourth figure locates the eight IBM Table 5 blocks whose matched State Mine Block Summaries passed the alpha.9 geometry gates. It also makes the incomplete review scope explicit: three reviewed conflicts are withheld and 86 IBM rows remain unreviewed. The map is generated by `scripts/plot_ibm_auction_mbs_geometries.py`.
...
| IBM 2024 mining-lease and auction-granted concession context | Implemented as context; spatial reconciliation started | National/state/mineral/sector/area-band distributions and 97 named blocks. Alpha.9 reviews eleven exact State MBS matches, admits eight validated source footprints and withholds three conflicts. All remain excluded from model evidence and current-status inference. |
...
| `outputs/india_ibm_auctioned_concession_mbs_match_audit_2023_24.csv` | 97 | Complete IBM-row audit of the alpha.9 State MBS review scope, selected documents, hashes, area conflicts and geometry-admission outcomes. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.csv` | 8 | Validated source footprints with normalized materials, drilling, resources, grade, climate, terrain, hydrology, access, geometry checks and document provenance. Context only. |
| `outputs/india_ibm_auctioned_concession_geometries_2023_24.geojson` | 8 | GeoJSON representation of the same eight validated source footprints. |
...
| `outputs/data_dictionary.csv` | 1,105 | Field definitions, units and missing-value policies. |
...
- **Started in alpha.9:** link eleven IBM 2023-24 auction rows to exact State Mine Block Summaries, admit eight validated source footprints, and quarantine three source conflicts.
